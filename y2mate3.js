...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  let filesizeee = list[quality]
  let id = /var k__id = "(.*?)"/.exec(document.body.innerHTML) || ['', '']
  let thumbbb = document.querySelector('img').src
  let tttitle = document.querySelector('b').innerHTML
  ...
  let KB = parseFloat(filesizeee) * (1000 * /MB$/.test(filesizeee))
  return {
    dl_linkkk: /<a.+?href="(.+?)"/.exec(json2.result)[1],
    thumbbb,
    tttitle,
    filesizeFff: filesizeee,
    filesizeee: KB
  }
}
...
module.exports = {
   ...
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  ytaa(url, server = 'en68') { return yt(url, '128kbps', 'mp3', '128', server) },
  /**
   * Download YouTube Video as Video via y2mate
   * @param {String} url YouTube Video URL
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  ytvv(url, server = 'en68') { return yt(url, '360p', 'mp4', '360', server) },
  serversss: ['id4', 'en60', 'en61', 'en68']
}
