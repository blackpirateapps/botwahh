...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  let filesizee = list[quality]
  let id = /var k__id = "(.*?)"/.exec(document.body.innerHTML) || ['', '']
  let thumbb = document.querySelector('img').src
  let ttitle = document.querySelector('b').innerHTML
  ...
  let KB = parseFloat(filesizee) * (1000 * /MB$/.test(filesizee))
  return {
    dl_linkk: /<a.+?href="(.+?)"/.exec(json2.result)[1],
    thumbb,
    ttitle,
    filesizeFf: filesizee,
    filesizee: KB
  }
}
